...
def generate_frames():
    ...
    while True:
        ...
        hf_emotion = str(hf.get("emotion") or "OFF")
        hf_score = hf.get("score") or 0.0
        hf_line = f"HF EMOTION: {hf_emotion}"
        ...
        cv2.putText(
            ...
            cv2.FONT_HERSHEY_SIMPLEX,
            0.6,
            (180, 220, 255) if hf.get("enabled") else (160, 160, 160),
            2,
        )
        if hf.get("enabled"):
            backend = hf.get("backend") or "huggingface"
            device_name = f"MEDIAPIPE + HF ({backend})"
        else:
